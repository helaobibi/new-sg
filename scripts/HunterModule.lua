...
local Growl = SpellBook:GetSpell(27047)                   -- 低吼
local Bite = SpellBook:GetSpell(52473)                    -- 撕咬
local Misdirection = SpellBook:GetSpell(34477)           -- 误导
...
-- 误导（给宠物）
-- PetControlAPL:AddSpell(
--     Misdirection:CastableIf(function(self)
--         return Pet:Exists()
--             and Pet:IsAlive()
--             and Player:IsAffectingCombat()
--             and Target:Exists()
...
--             and not Player:IsChanneling()
--             and not self:IsOnCooldown()
--             and not Player:GetAuras():FindMy(self):IsUp()
--     end):SetTarget(Pet)
-- )
...
HunterModule:Sync(function()
    ...
    -- if not TargetTarget:IsPet() then
    --     return
    -- end
    -- 战斗中切目标
