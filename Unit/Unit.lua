...
---@return string | nil
function Unit:GetTargetGUID()
    return tostring(UnitTarget(self:GetOMToken()))
end

-- Get the unit's target as a Unit object
---@return Unit | nil
function Unit:GetTarget()
    return Bastion.UnitManager:GetObject(self:GetTargetGUID()) or Bastion.UnitManager:Get('none')
end
...
-- Get the number of enemies in a given range of the unit and cache the result for 0.5 seconds
---@param range number
---@return number
...
-- 移动到目标背后的简单版本
---@param distance? number 距离目标背后的距离，如果不指定则基于战斗范围自动计算
---@return boolean 是否成功执行移动
function Unit:MoveToTargetBehind(distance)
    -- 检查目标是否存在
    ...
    local targetRotation = self:GetFacing()

    if not targetPos or not targetRotation then
        return false
    end

    -- 如果没有指定距离，则基于战斗范围计算精确距离
    if not distance then
        local playerCombatReach = ObjectCombatReach('player') or 1.5
        local targetCombatReach = ObjectCombatReach(self:GetOMToken()) or 1.5

        -- 基于InMelee函数的计算方法，确保在近战范围内但不会太近
        -- 使用近战范围计算公式：max((playerCR + 1.3333) + targetCR, 5.0) + 1.0
        -- 计算近战范围并在其基础上增加距离，确保在背后但仍在攻击范围内
        distance = math.max((playerCombatReach + 1.3333) + targetCombatReach, 5.0)
    end

    -- 使用RotateVector计算背后位置
    local behindAngle = targetRotation + math.pi
    local behindX, behindY, behindZ = RotateVector(targetPos.x, targetPos.y, targetPos.z, behindAngle, distance)

    -- 执行移动
    MoveToRaw(behindX, behindY, behindZ)
    return true
end
