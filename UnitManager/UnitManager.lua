...
function UnitManager:__index(k)
    ...
    if self.customUnits[k] then
        if not self.cache:IsCached(k) then
            ...
            self.cache:Set(k, self.customUnits[k].unit, 0.1)
        end
...
-- Create a custom unit and cache it for .2 seconds
---@param token string
---@param cb fun():Unit
---@return Unit
function UnitManager:CreateCustomUnit(token, cb)
    ...
    self.cache:Set(token, cachedUnit, 0.1)
...
---@param radius number
---@param range number
---@param minEnemyCount number|nil 最小敌人数量要求，如果找到的敌人群数量少于此值则返回nil（可选参数，默认为1）
---@return Vector3 | nil
function UnitManager:FindEnemiesCentroid(radius, range, minEnemyCount)
    -- 设置默认的最小敌人数量为1，保持向后兼容性
    minEnemyCount = minEnemyCount or 1
...
-- Find the actual enemy unit closest to the centroid of the most dense area of enemies
---@param radius number
---@param range number
---@param minEnemyCount number|nil 最小敌人数量要求，如果找到的敌人群数量少于此值则返回nil（可选参数，默认为1）
---@return Unit | nil
function UnitManager:GetEnemyClosestToCentroid(radius, range, minEnemyCount)
    -- 设置默认的最小敌人数量为1，保持向后兼容性
    minEnemyCount = minEnemyCount or 1

    -- 找到最密集的敌人群
    local unit, enemies = self:GetEnemiesWithMostEnemies(radius)
    if unit == nil or #enemies == 0 then
        return nil
    end
    ...
    local centroid = Bastion.Vector3:New(0, 0, 0)
    for i = 1, #enemies do
        local pos = enemies[i]:GetPosition()
    ...
    for i = 1, #enemies do
        ...
        local distance = enemy:GetPosition():Distance(centroid)
